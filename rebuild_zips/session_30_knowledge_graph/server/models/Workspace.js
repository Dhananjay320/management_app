...
const documentSchema = new mongoose.Schema({
  ...
  // Session 30 (N5): knowledge graph backlinks. Stores the outbound refs
  // parsed from the document body ([[Title]] style), so the "Linked from"
  // panel of a target document can be built via a reverse query without
  // having to text-scan every document.
  //
  // We store each link as (targetId, titleAtLinkTime) — the title is saved
  // at link time so broken links still show a useful label if the target
  // was renamed/deleted.
  backlinksOut: [{
    _id: false,
    targetId: { type: mongoose.Schema.Types.ObjectId, ref: 'WorkspaceDocument' },
...
// Session 30 (N5): reverse backlinks query — "who links to me?"
documentSchema.index({ 'backlinksOut.targetId': 1 });
