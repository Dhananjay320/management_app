...
// Session 17 C2: user-aware "today". Attendance must be keyed by the user's
// local date, not UTC. An IST user marking attendance at 01:00 local is on
// a different UTC date — without this fix the record went on the wrong day.
function todayStrFor(user) {
  return userToday(user);
}

// Kept for any legacy call sites that still reference it (module-scoped fn
// that uses UTC). New code should pass the user through todayStrFor().
function todayStr() {
...
router.get('/today', protect, async (req, res) => {
  try {
    const record = await Attendance.findOne({ user: req.user._id, date: todayStrFor(req.user) });
    res.json(record || { status: 'not_marked', date: todayStrFor(req.user) });
  } catch (err) {
...
router.post('/mark-entry', protect, async (req, res) => {
  try {
    ...
    const date = todayStrFor(req.user);
...
// Session 11 C9: respect adminScope.teams if this admin is scoped to specific
// teams, and optional ?team=X query to narrow to a single team.
router.get('/team', protect, requirePower('attendance', 'viewTeam'), async (req, res) => {
  try {
    const date = req.query.date || todayStrFor(req.user);
    const requestedTeam = req.query.team || null;

    // Determine which user IDs are visible to this admin
    let userIdFilter = null;
    if (req.user.role !== 'main_admin') {
      const scopedTeams = (req.user.adminScope?.teams || []).map(String);
      if (requestedTeam) {
        const { canAccessTeam } = require('../utils/teamAccess');
        const ok = await canAccessTeam(req.user, requestedTeam);
        if (!ok) return res.status(403).json({ error: 'You cannot view attendance for that team.' });
        const teamUsers = await User.find({ teams: requestedTeam, isActive: true }).select('_id');
        userIdFilter = teamUsers.map(u => u._id);
      } else if (scopedTeams.length > 0) {
        // Scoped admin without specific team param: narrow to their scope
        const teamUsers = await User.find({ teams: { $in: scopedTeams }, isActive: true }).select('_id');
        userIdFilter = teamUsers.map(u => u._id);
      }
    } else if (requestedTeam) {
      // main_admin with explicit team filter
      const teamUsers = await User.find({ teams: requestedTeam, isActive: true }).select('_id');
      userIdFilter = teamUsers.map(u => u._id);
    }

    const recordFilter = { date };
    if (userIdFilter) recordFilter.user = { $in: userIdFilter };

    const records = await Attendance.find(recordFilter)
      .populate('user', 'name email jobTitle avatar')
      .sort({ entryTime: 1 });

    // Also get users who haven't marked (within the same scope)
    ...
    if (userIdFilter) unmarkedFilter._id = { $in: userIdFilter, $nin: markedUserIds };

    const unmarked = await User.find(unmarkedFilter).select('name email jobTitle');

    res.json({ marked: records, unmarked, date });
  } catch (err) {
    console.error('[attendance] team view failed', err);
    res.status(500).json({ error: 'Server error.' });
  }
});
...
router.post('/manual-mark', protect, requirePower('attendance', 'markManually'), async (req, res) => {
  try {
    ...
    const record = await Attendance.findOneAndUpdate(
      { user: userId, date: date || todayStrFor(req.user) },
      {
