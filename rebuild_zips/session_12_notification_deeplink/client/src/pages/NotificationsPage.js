    ...
    <div className="notif-layout">
      ...
      {notifications.length === 0 ? (
        ...
        <div className="notif-list">
          {notifications.map(n => {
            const hasLink = !!pathForNotification(n);
            return (
            <div
              key={n._id}
              className={`notif-item ${!n.isRead ? 'unread' : ''} ${n.isEmergency ? 'emergency' : ''} ${hasLink ? 'has-link' : ''}`}
              onClick={() => followNotification(n)}
              title={hasLink ? 'Click to open' : undefined}
              role={hasLink ? 'link' : undefined}
              tabIndex={hasLink ? 0 : undefined}
              onKeyDown={(e) => { if (hasLink && (e.key === 'Enter' || e.key === ' ')) { e.preventDefault(); followNotification(n); } }}
            >
              ...
              <div className="notif-content">
                ...
                <div className="notif-meta">
                  ...
                  {hasLink && <span className="notif-deeplink-hint" aria-hidden="true">↗</span>}
                </div>
              </div>
