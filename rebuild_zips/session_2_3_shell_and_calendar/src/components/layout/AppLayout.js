...
const PRIMARY_NAV = [
  ...
  { key: 'tasks',         icon: Icon.CheckCircle,   label: 'Tasks',         path: '/tasks' },
  { key: 'messages',      icon: Icon.MessageSquare, label: 'Messages',      path: '/messages', unread: true },
  { key: 'attendance',    icon: Icon.Clock,         label: 'Attendance',    path: '/attendance' },
  { key: 'meetings',      icon: Icon.Users,         label: 'Meetings',      path: '/meetings' },
  { key: 'workspace',     icon: Icon.Folder,        label: 'Workspace',     path: '/workspace' },
  { key: 'email',         icon: Icon.Mail,          label: 'Email',         path: '/email' },
...
function NavItem({ item, active, onClick }) {
  ...
  return (
    ...
    >
      ...
      {item.unread && (
        <span className="ad-nav-item__unread" aria-label="Has unread">3</span>
      )}
    </button>
  );
}
...
export default function AppLayout() {
  ...
  // Real notification/message counts will flow in from Session 12 (C3). For now
  // placeholder until then — a sensible default so the UI isn't empty.
  const notifBadge = 4;
        ...
        <div className="ad-topbar ad-glass ad-glass--strong ad-glass--elevated">
          ...
          <div className="ad-topbar__left">
            ...
            <SearchBar
              ...
              // onShortcut / onEnter wired in Session 15 (C10 — global search UI)
            />
          </div>
          ...
          <div className="ad-topbar__right">
            ...
            <IconButton title="Notifications" badge={notifBadge} onClick={() => go('/notifications')}>
              <Icon.Bell size={16} />
            </IconButton>
