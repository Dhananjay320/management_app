...
export default function UserProfilePage() {
  ...
  const [gam, setGam] = useState(null);  // Session 28 gamification stats
  ...
  const refreshGam = async () => {
    try {
      const { data } = await api.get(`/gamification/user/${userId}`);
      setGam(data);
    } catch {}
  };
  useEffect(() => { refreshCounts(); refreshGam(); /* eslint-disable-next-line */ }, [userId]);
      ...
      <GlassPanel elevated className="ad-up__hero ad-enter">
        ...
        <div className="ad-up__info">
          ...
          <div className="ad-up__stats">
            ...
            {gam && (
              <Link to="/achievements" className="ad-up__stat">
                <span className="ad-up__stat-value">Lv {gam.level}</span>
                <span className="ad-up__stat-label">{gam.xp.toLocaleString()} XP</span>
              </Link>
            )}
            {gam && gam.unlockedCount > 0 && (
              <Link to="/achievements" className="ad-up__stat">
                <span className="ad-up__stat-value">🏅 {gam.unlockedCount}</span>
                <span className="ad-up__stat-label">badges</span>
              </Link>
            )}
            {profile.team?.name && (
