...
router.get('/', protect, async (req, res) => {
  try {
    ...
    if (view === 'my') filter.assignees = req.user._id;
      ...
      // Session 11 C9: caller must be a member/lead/scoped admin of that team
      const ok = await canAccessTeam(req.user, team);
...
router.post('/', protect, async (req, res) => {
  try {
    // Session 11 C9: if the task is team-scoped, the creator must be able to
    // access that team (member, lead, or scoped admin).
    if (req.body.team) {
      try { await assertTeamMember(req.user, req.body.team); }
  ...
  try {
    ...
    if (updates.status && updates.status !== task.status) {
      ...
      if (updates.status === 'done') {
        ...
        // Session 28 (N8): award XP + check achievements. Best-effort.
        try {
          const { awardXpAndCheck } = require('../utils/gamificationEngine');
          await awardXpAndCheck(req.user, 'task_completed');
        } catch {}
      }
    }
