...
router.post('/', protect, async (req, res) => {
  try {
    ...
    // Session 28 (N8): award XP to follower + check both users' achievements.
    try {
      const { awardXpAndCheck } = require('../utils/gamificationEngine');
      await awardXpAndCheck(req.user, 'follow_created');
      // The followed user just gained a follower — check their badges too.
      const User = require('../models/User');
      const target = await User.findById(userId);
      if (target) await awardXpAndCheck(target, 'follower_added');
    } catch {}
