...
const powerSchema = new mongoose.Schema({
  ...
  // ─── Content Hub (Session 29 N7) ─────────────────────────────────────────
  // Users with `publish` can create, edit, and delete content items (tutorials,
  // guides, announcements). Everyone with a login can read.
  content: {
...
const userSchema = new mongoose.Schema({
  ...
  // Session 28 (N8): Gamification stats. XP is cumulative; level and streak
  // are derived but cached on the user for fast profile/leaderboard queries.
  // Level is recalculated whenever XP changes (utils/gamificationEngine).
  // Streak counters are updated by relevant workflows (mood, login, etc).
  xp:           { type: Number, default: 0 },
  level:        { type: Number, default: 1 },
  moodStreak:   { type: Number, default: 0 },
  lastMoodDate: { type: String, default: '' }    // YYYY-MM-DD used to detect streak breaks
}, {
  timestamps: true
});
