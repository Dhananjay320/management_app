...
export default function Tasks() {
  ...
  // Session 14+18: use useFetchSafe for consistent loading/error/retry.
  ...
  // Session 12+18 deep-link: if URL has ?highlight=<id>, open that task.
  // Clears the param after handling so back/forward doesn't re-trigger.
  useEffect(() => {
    if (!highlightId || loading) return;
    const match = tasks?.find?.(t => String(t._id) === String(highlightId));
    if (match) {
      openTask(highlightId);
      // Scroll & flash the card once its row becomes visible
      setTimeout(() => {
        if (highlightRef.current) {
          highlightRef.current.scrollIntoView({ behavior: 'smooth', block: 'center' });
          highlightRef.current.classList.add('ad-tasks-row--flash');
          setTimeout(() => highlightRef.current?.classList.remove('ad-tasks-row--flash'), 1800);
        }
      }, 200);
      // Strip param so subsequent nav is clean
  ...
  return (
    <div className="ad-tasks">
      {/* ─── Hero header ───────────────────────────────────────────── */}
      <header className="ad-tasks__head ad-enter">
        <div className="ad-tasks__head-left">
          <h1 className="ad-tasks__title">
            {tab === 'todo' ? 'To-Do List'
              : tab === 'create' ? 'Create Task'
              : tab === 'detail' ? 'Task Detail'
              : <>Your <GradientText>tasks</GradientText></>}
          </h1>
          {tab === 'tasks' && (
            <p className="ad-tasks__sub">
              {loading ? 'Loading…'
                : totalTasks === 0 ? 'No tasks yet. Create one to get started.'
                : `${totalTasks} task${totalTasks === 1 ? '' : 's'} · ${doneTasks} done`}
            </p>
          )}
        </div>
        <div className="ad-tasks__head-right">
          <SegmentedControl
            value={tab === 'detail' || tab === 'create' ? 'tasks' : tab}
            onChange={(k) => { setTab(k); setSelectedTask(null); }}
            options={[{ key: 'tasks', label: 'Tasks' }, { key: 'todo', label: 'To-Do' }]}
          />
          {tab === 'tasks' && (
      ...
      {tab === 'tasks' && (
        <>
          {/* ─── View filters ─────────────────────────────────────── */}
          <div className="ad-tasks__filters ad-enter" style={{ animationDelay: '60ms' }}>
            <SegmentedControl
              value={view}
              onChange={setView}
              options={[
          ...
          {loading ? (
            <GlassPanel elevated className="ad-tasks__state">Loading…</GlassPanel>
          ) : error ? (
            <ErrorState error={error} onRetry={refetch} />
          ) : totalTasks === 0 ? (
            <GlassPanel elevated className="ad-tasks__state">
              <div className="ad-tasks__empty-icon"><Icon.CheckCircle size={24} /></div>
              <div className="ad-tasks__empty-title">No tasks yet</div>
              <div className="ad-tasks__empty-sub">Create a task to get started.</div>
            </GlassPanel>
          ) : (
            Object.entries(grouped).map(([pri, items]) => items.length > 0 && (
              <div key={pri} className="task-priority-section ad-enter" style={{ animationDelay: '100ms' }}>
                <div className="task-priority-header">
                  <span>{PRIORITY_CONFIG[pri].label}</span>
                  <span className="task-priority-count">{items.length} task{items.length === 1 ? '' : 's'}</span>
                </div>
                {items.map(task => (
                  <div
                    key={task._id}
                    ref={String(task._id) === String(highlightId) ? highlightRef : null}
                    className={`ad-tasks-row ${String(task._id) === String(highlightId) ? 'ad-tasks-row--target' : ''}`}
                  >
                    <TaskCard task={task} onClick={() => openTask(task._id)} />
                  </div>
                ))}
              </div>
            ))
      ...
      {tab === 'create' && <CreateTask onBack={() => setTab('tasks')} onCreated={() => { setTab('tasks'); refetch(); }} />}
