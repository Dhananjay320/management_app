...
const PRIMARY_NAV = [
  ...
  { key: 'whiteboards',   icon: Icon.Folder,        label: 'Whiteboards',   path: '/whiteboards' },
...
export default function AppLayout() {
  ...
  return (
    <div className={`ad-shell ${drawerOpen ? 'ad-shell--drawer-open' : ''}`}>
      {/* Session 33: Electron titlebar (no-op in browser) */}
      <ElectronTitleBar />
      ...
      {/* Session 34: auto-update toast (no-op in browser) */}
      <UpdateToast />
    </div>
  );
}
