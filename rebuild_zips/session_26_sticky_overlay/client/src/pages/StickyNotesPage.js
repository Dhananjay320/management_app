...
export default function StickyNotesPage() {
  ...
  // Session 26 (N1): pin/unpin a note to the floating overlay
  const togglePin = async (id, currentPinned) => {
    try {
      await api.put(`/sticky-notes/${id}/pin`, { pinned: !currentPinned });
      setNotes(prev => prev.map(n => n._id === id ? { ...n, overlayPinned: !currentPinned } : n));
      // Tell the overlay to reload
      window.dispatchEvent(new CustomEvent('stickynote:pin-changed'));
    } catch {}
  };
                  ...
                  {isOwner(note) && (
                    <>
                      <button
                        ...
                        title={note.overlayPinned ? 'Unpin from screen' : 'Pin to screen (floating note)'}
                        onClick={() => togglePin(note._id, note.overlayPinned)}
                        style={{ color: note.overlayPinned ? '#8B5CF6' : undefined }}
                      >
                        {note.overlayPinned ? '📍' : '📌'}
                      </button>
                      <button
