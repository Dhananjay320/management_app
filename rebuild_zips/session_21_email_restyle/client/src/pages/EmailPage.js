...
export default function EmailPage() {
  ...
  // Session 21 deep-link: ?highlight=<emailId> opens that email in the right
  // pane AND flashes its row in the middle-panel list. Used by Session 12
  // notifications ("New email from <person>") and Session 15 command palette.
  const highlightEmailId = searchParams.get('highlight');
  useEffect(() => {
    if (!highlightEmailId || emails.length === 0) return;
    const match = emails.find(e => String(e._id) === String(highlightEmailId));
    if (match) {
      selectEmail(match);
      setTimeout(() => {
        if (highlightRef.current) {
          highlightRef.current.scrollIntoView({ behavior: 'smooth', block: 'center' });
          highlightRef.current.classList.add('ad-email-row--flash');
          setTimeout(() => highlightRef.current?.classList.remove('ad-email-row--flash'), 1800);
        }
      }, 200);
  ...
  // ─── Render ───
  // Mobile-stack state: which pane is "visible" on narrow screens.
  // Pattern mirrors Messages (S19): sidebar → list → detail flows via a
  // CSS class on the outer layout. Clicking a row advances the pane;
  // the back buttons on list + detail retreat.
  const mobileView = activeEmail ? 'detail' : 'list';

  return (
    <div className={`email-layout ad-email-layout ad-email-layout--${mobileView}`}>
      {/* ═══ Left Sidebar ═══ */}
          ...
          {activeFolder === 'drafts' ? (
            ...
            loading ? (
              ...
              filteredEmails.map(email => (
                <div
                  key={email._id}
                  ref={String(email._id) === String(highlightEmailId) ? highlightRef : null}
                  className={`email-list-item ad-email-row ${activeEmail?._id === email._id ? 'active' : ''} ${!email.isRead ? 'unread' : ''}`}
      ...
      {activeEmail ? (
        <div className="email-detail-panel">
          <div className="email-detail-header">
            {/* Session 21 C8: mobile back button — returns to email list */}
            <button
              type="button"
              className="ad-email-back"
          ...
          <div className="email-detail-body">
            <div
              ...
              /* S4 (Session 5): sanitize email HTML through DOMPurify before render.
                 Previously this was direct innerHTML which allowed <script>, event
                 handlers, and javascript: URLs — a critical XSS vector. */
              dangerouslySetInnerHTML={{
                __html: sanitizeHtml(activeEmail.bodyHtml || activeEmail.bodyText?.replace(/\n/g, '<br/>') || '')
              }}
            />
          </div>
        </div>
