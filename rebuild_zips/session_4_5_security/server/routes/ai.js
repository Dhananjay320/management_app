...
router.post('/activate', protect, async (req, res) => {
  try {
    ...
    if (!parsed) return res.status(400).json({ error: 'Invalid activation code format or expiry.' });

    // S10: expiry now comes pre-parsed as a valid Date from parseActivationCode.
