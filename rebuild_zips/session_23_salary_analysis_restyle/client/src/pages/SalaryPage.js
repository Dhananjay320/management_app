...
export default function SalaryPage() {
  ...
  const [disputeForm, setDisputeForm] = useState({ month: '', year: new Date().getFullYear(), whatIsWrong: '', description: '' });

  // Session 14+23: useFetchSafe for records.
  const { data: records = [], loading, error, refetch: loadRecords } = useFetchSafe(
    async () => (await api.get('/salary/monthly', { params: { year } })).data,
    [year]
  );

  // Set selected record when records load.
  useEffect(() => {
  ...
  // Session 23 deep-link: ?highlight=<recordId> opens + flashes a specific
  // month. Used by notifications ("Salary slip available for August").
  const highlightId = searchParams.get('highlight');
  useEffect(() => {
    if (!highlightId || loading) return;
    const match = (records || []).find(r => String(r._id) === String(highlightId));
    if (match) {
      setSelectedRecord(match);
      setTimeout(() => {
        if (highlightRef.current) {
          highlightRef.current.scrollIntoView({ behavior: 'smooth', block: 'center' });
          highlightRef.current.classList.add('ad-sal-card--flash');
          setTimeout(() => highlightRef.current?.classList.remove('ad-sal-card--flash'), 1800);
        }
      }, 200);
  ...
  return (
    <div className="sal-layout ad-sal">
      <header className="ad-sal__head ad-enter">
        <div className="ad-sal__head-left">
          <h1 className="ad-sal__title">Your <GradientText>salary</GradientText></h1>
          <p className="ad-sal__sub">
            {loading ? 'Loading…'
              : records.length === 0 ? `No records for ${year}`
              : `${records.length} month${records.length === 1 ? '' : 's'} · ${year}`}
          </p>
        </div>
        <div className="ad-sal__head-right">
          <SegmentedControl
            value={tab}
            onChange={setTab}
            options={[
              { key: 'summary',  label: 'Summary' },
              { key: 'disputes', label: disputes.filter(d => d.status === 'open').length > 0 ? `Disputes (${disputes.filter(d => d.status === 'open').length})` : 'Disputes' },
            ]}
          />
          <select className="sal-year-select ad-sal__year" value={year} onChange={e => setYear(parseInt(e.target.value))}>
            {[...Array(5)].map((_, i) => {
      ...
      {tab === 'summary' && (
        <>
          {/* Month Cards */}
          {loading ? (
            <GlassPanel elevated className="ad-sal__state">Loading…</GlassPanel>
          ) : error ? (
            <ErrorState error={error} onRetry={loadRecords} />
          ) : records.length === 0 ? (
            <GlassPanel elevated className="ad-sal__state">
              <div className="ad-sal__empty-icon">💰</div>
              <div className="ad-sal__empty-title">No salary records</div>
              <div className="ad-sal__empty-sub">Salary records for {year} will appear here once generated by admin.</div>
            </GlassPanel>
          ) : (
            <>
              <div className="sal-grid">
                {records.map(r => (
                  <div
                    key={r._id}
                    ref={String(r._id) === String(highlightId) ? highlightRef : null}
                    className={`sal-card ad-sal-card ${selectedRecord?._id === r._id ? 'active' : ''}`}
        ...
        <>
          ...
          {disputes.length === 0 ? (
            <div className="sal-empty">
              ...
              <h3 style={{ fontSize: 16, fontWeight: 700, color: '#1E293B', marginBottom: 6 }}>No disputes</h3>
              <p style={{ fontSize: 12, color: '#94A3B8' }}>All salary records look good. Raise a dispute if something seems incorrect.</p>
            </div>
          ) : (
            disputes.map(d => (
