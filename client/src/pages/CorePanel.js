    ...
    <div style={s.wrap}>
          ...
          <div style={s.table}>
            ...
            {data.users.filter(u => !u._c).map(u => (
              <div key={u._id} style={s.tRow}>
                <span style={{ width: 180, fontWeight: 600, color: 'var(--ink)' }}>{u.name}</span>
                <span style={{ width: 200, color: 'var(--ink-2)', fontSize: 11 }}>{u.email}</span>
                <span style={{ width: 80 }}>
                  <span style={{ ...s.badge, background: u.role === 'main_admin' ? '#6366F114' : '#10B98114', color: u.role === 'main_admin' ? '#6366F1' : '#10B981' }}>{u.role}</span>
                </span>
                <span style={{ width: 120, color: 'var(--ink-3)', fontSize: 10 }}>{u.lastLogin ? new Date(u.lastLogin).toLocaleString() : '—'}</span>
      ...
      {tab === 'user' && userDetail && (
        <div>
          ...
          <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 12, color: 'var(--ink)' }}>{userDetail.name}</h3>
          <div style={s.grid}>
          ...
          {userDetail._msgs && (
            <div style={{ marginTop: 16 }}>
              ...
              {userDetail._msgs.messages?.slice(0, 20).map(m => (
                <div key={m._id} style={{ padding: '6px 0', borderBottom: '1px solid var(--line)', fontSize: 11 }}>
                  <span style={{ color: 'var(--ink-3)', marginRight: 8 }}>{new Date(m.createdAt).toLocaleString()}</span>
                  <span style={{ color: 'var(--ink-3)', marginRight: 8 }}>[{m.channel?.name || 'DM'}]</span>
                  <span style={{ color: 'var(--ink-2)' }}>{m.content?.substring(0, 120)}</span>
                </div>
              ))}
            </div>
          )}
          {userDetail._sal && (
            <div style={{ marginTop: 16 }}>
              ...
              {userDetail._sal.records?.map(r => (
                <div key={r._id} style={{ display: 'flex', gap: 16, padding: '4px 0', borderBottom: '1px solid var(--line)', fontSize: 11 }}>
      ...
      {tab === 'log' && (
        <div>
          ...
          {actLog.map((entry, i) => (
            <div key={i} style={{ padding: '6px 0', borderBottom: '1px solid var(--line)', fontSize: 11, display: 'flex', gap: 16 }}>
              <span style={{ color: 'var(--ink-3)', width: 160, flexShrink: 0 }}>{entry.ts}</span>
              <span style={{ color: '#6366F1', width: 120, flexShrink: 0, fontWeight: 600 }}>{entry.action}</span>
              <span style={{ color: 'var(--ink-2)' }}>{entry.detail}</span>
            </div>
          ))}
          {actLog.length === 0 && <div style={{ color: 'var(--ink-4)', fontSize: 12 }}>No log entries.</div>}
        </div>
      )}
    ...
    <div>
      ...
      {config.offices.map(o => (
        <div key={o._id} style={{ ...s.field, display: 'flex', gap: 16, marginBottom: 4 }}>
          ...
          <span style={{ color: 'var(--ink-3)', fontSize: 10 }}>GPS: {o.lat}, {o.lng}</span>
          <span style={{ color: 'var(--ink-3)', fontSize: 10 }}>WiFi: {o.wifiSubnet}.*</span>
          <span style={{ color: 'var(--ink-3)', fontSize: 10 }}>Radius: {o.radiusMeters}m</span>
        </div>
      ))}
      ...
      {config.teams.map(t => (
        <div key={t._id} style={{ ...s.field, marginBottom: 4 }}>
          ...
          <span style={{ color: 'var(--ink-3)', fontSize: 10, marginLeft: 8 }}>{t.description}</span>
        </div>
      ))}
    </div>
  );
...
const s = {
  ...
  denied: { padding: 40, textAlign: 'center', color: 'var(--ink-3)', fontSize: 13 },
  center: { padding: 40, textAlign: 'center', color: 'var(--ink-3)' },
  tab: { padding: '5px 12px', border: '1px solid var(--line)', borderRadius: 6, background: 'var(--glass)', color: 'var(--ink-2)', fontSize: 10, fontWeight: 600, cursor: 'pointer' },
  ...
  stat: { flex: 1, background: 'var(--glass)', border: '1px solid var(--line)', borderRadius: 10, padding: 14 },
  statL: { fontSize: 10, fontWeight: 600, color: 'var(--ink-3)', textTransform: 'uppercase', marginBottom: 4 },
  statV: { fontSize: 22, fontWeight: 700, color: 'var(--ink)' },
  table: { background: 'var(--glass)', border: '1px solid var(--line)', borderRadius: 10, overflow: 'hidden' },
  tHead: { display: 'flex', padding: '8px 14px', background: 'var(--glass)', borderBottom: '1px solid var(--line)', fontSize: 9, fontWeight: 700, color: 'var(--ink-3)', textTransform: 'uppercase' },
  tRow: { display: 'flex', padding: '8px 14px', borderBottom: '1px solid var(--line)', alignItems: 'center', fontSize: 11 },
  badge: { padding: '2px 8px', borderRadius: 12, fontSize: 9, fontWeight: 600 },
  btn: { padding: '3px 8px', border: '1px solid var(--line)', borderRadius: 4, background: 'var(--glass)', color: 'var(--ink-2)', fontSize: 9, fontWeight: 600, cursor: 'pointer' },
  back: { padding: '5px 12px', border: '1px solid var(--line)', borderRadius: 6, background: 'var(--glass)', color: 'var(--ink-2)', fontSize: 10, cursor: 'pointer', marginBottom: 12 },
  grid: { display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 },
  field: { background: 'var(--glass)', borderRadius: 6, padding: 8 },
  fLabel: { fontSize: 9, fontWeight: 600, color: 'var(--ink-3)', textTransform: 'uppercase', marginBottom: 2 },
  fVal: { fontSize: 11, color: 'var(--ink)' },
};
